...
var LayerToIgnore : LayerMask = -1; // Layer to ignore, when do raycasting
var layerMask : int;
...
function FindPath(start : Vector3, dest : Vector3)
{
    ...
    if(resultPath == -1)
    {
        ...
        while(opendList.length != 0)
        {
			...
			var f : float = (opendList[0] as Waypoint).af;
			var index : int = 0;
			for(var i : int = 0; i < opendList.length; i++)
			{
			...
			if((opendList[index] as Waypoint).childList.length == 0)
			{
			...
			var LastNode : Waypoint = closedList[closedList.length-1];
			for (var child : Waypoint in LastNode.childList)
            {
                var skip : int = 0;
                ...
                if(skip == 0)
                {
					...
					for (var node : Waypoint in opendList)
					{
						...
						{
							skip = 1;
							dist = Vector3.Distance((closedList[closedList.length-1] as Waypoint).Pos, child.Pos);
							if(((closedList[closedList.length-1] as Waypoint).ag + dist) < child.ag)
							{
								...
								child.ag = (closedList[closedList.length-1] as Waypoint).ag + dist;
					...
					{
						child.creator = closedList[closedList.length-1];
						child.ag = (closedList[closedList.length-1] as Waypoint).ag + Vector3.Distance((closedList[closedList.length-1] as Waypoint).Pos, child.Pos);
						child.ah = Vector3.Distance(child.Pos, dest);
						child.af = child.ag + child.ah;
