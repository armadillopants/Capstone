...
public class SellableItemDisplayer : MonoBehaviour {
	...
	public void AddToDisplay(DisplayItem item){
		...
		for(int i=0; i<itemsToDisplay.Count; i++){
			Debug.Log(itemsToDisplay[i].item);
		}
	}
	
	public void RemoveFromDisplay(GameObject item){
		for(int i=0; i<itemsToDisplay.Count; i++){
	...
	void OnGUI(){
		for(int i=0; i<itemsToDisplay.Count; i++){
			...
			if(GUI.Button(new Rect(guiLoc.x,guiLoc.y*i,itemsToDisplay[i].windowSize.x,itemsToDisplay[i].windowSize.y),content,style)){
				Debug.Log(i);
				// Get Method
				MethodInfo method;
				try {
