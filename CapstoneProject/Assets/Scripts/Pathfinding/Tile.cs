...
public class Tile {
	...
	public Tile(Vector2 id, Vector3 scale, Vector3 pos, GameObject cube, GameObject sphere, bool walkable){
		...
		this.sphere.renderer.enabled = true;
		
		// Postion and scale
		this.cube.transform.localScale = scale;
		this.cube.transform.position = pos + new Vector3(-3,0,-3);
		
		this.sphere.transform.localScale = scale;
		this.sphere.transform.position = new Vector3(pos.x, pos.y+1, pos.z) + new Vector3(-3,0,-3);
